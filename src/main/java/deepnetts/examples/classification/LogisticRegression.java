...
public class LogisticRegression {
    ...
    public static void main(String[] args) throws IOException {
        ...
        TrainTestSplit trainTest = DataSets.trainTestSplit(dataSet, 0.7);

        FeedForwardNetwork neuralNet = FeedForwardNetwork.builder()
                .addInputLayer(numInputs)                               // size of the input layer should be same as number of inputs
                .addOutputLayer(numOutputs, ActivationType.SIGMOID)     // size of the output layer should be same as the number of outputs, and with SIGMOID (logistic) function since we're doing logistic regression
        ...
        neuralNet.train(trainTest.getTrainingSet());

        EvaluationMetrics testResults = neuralNet.test(trainTest.getTestSet());
